...
bool readInventory(BufferReader& reader, Inventory& inventory) {
    ...
    if (!reader.readInt(weightLimit) || !reader.readInt(slotLimit) || !reader.readU32(count) || weightLimit < 0 ||
        slotLimit < 0 || count > kMaximumMissionInventoryItems) {
        return false;
    }
...
bool readWar(BufferReader& reader, WarState& war) {
    ...
    if (!reader.readBool(war.active) || !readEnum(reader, war.enemy, TribeId::Player, TribeId::Blackstone) ||
        ...
        !reader.readInt(war.craftsmanshipPower) || !reader.readU32(count) || count > kMaximumLockedWarEquipment)
        return false;
...
bool readGameState(BufferReader& reader, GameState& state, std::string& error, const bool containsItemSerial) {
    ...
    if (!reader.readU32(leadershipCount) || leadershipCount == 0U ||
        leadershipCount > kMaximumLeadershipHistoryEntries) {
        error = "存档的首领历史数量无效。";
        return false;
    }
    ...
    if (!reader.readU32(stockCount) || stockCount > kMaximumStockpileItems) {
        error = "存档仓库数量无效。";
        return false;
    }
...
bool deserializeFile(const std::string_view fileData, GameState& candidate, std::string& error,
    ...
    if (sourceVersion != nullptr) *sourceVersion = version;
    error.clear();
    return true;
}
...
bool serialize(const GameState& state, std::string& fileData, std::string& error) {
    return serializeFile(state, fileData, error);
}

/// 用途：校验魔数、版本、长度和校验和后解码 v5/v6 文件。输入：文件字节、候选状态、错误和可选来源版本。
/// 状态影响：仅成功写入 candidate。失败：candidate 保持原值；不变量：拒绝尾随字节、非法文本和不完整状态。
bool deserialize(const std::string_view fileData, GameState& candidate, std::string& error,
                 std::uint32_t* const sourceVersion) {
    return deserializeFile(fileData, candidate, error, sourceVersion);
}

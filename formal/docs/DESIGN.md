...
`SaveRepository` 负责 v7 文件读写、旧版本拒绝和七档摘要。`inspect()` 返回 `SaveSummary`，只读解析主档/备份/临时文件，不调用带恢复副作用的 `load()`。只有玩家选择档位才读取并恢复。v5/v6 及更旧存档不会自动迁移，原文件保持不变并提示需要新开局；文件不可访问时不选旧备份，以免误读较旧进度。

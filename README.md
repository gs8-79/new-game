...
游戏内：`1` 状态、`2` 地图、`5` 小队地图任务、`6` 外交、`7` 小队、`8` 结束季节、`9` 帮助。
...
旧经营面板的资源入口 `3`、`4`、`gather` 和 `scout` 已删除，不提供旧命令或旧存档兼容。

## 存档

`save 1` / `保存 1` 写入手动档；覆盖前需输入 `y` 或 `是`。`load 1` / `读取 1` 读取手动档，`load auto` 读取自动档。新局、季节结算、结局和正常退出时都会自动保存。

存档位于 `saves/game`，含 `slot1.sav` 至 `slot6.sav` 及 `autosave.sav`。当前格式版本为 4；其他版本会提示需要新开局，且不会修改原文件。

...
- `workforce/劳力` 查看分工，`assign/分配 <岗位> <人数>` 调整岗位。资源不再配置常驻资源队；工匠、医者、侦察、使者、营地守卫、住房和前哨守卫只区分未配置或已配置，使用 0 或 1。劳力、前哨守卫、驻军、已组建军队和出任务小队共同占用 `人口-2`，剩余人口就是本季行动力（最多7点）；人口损失造成超额时，必须先降低这些占用。
...
- `formarmy/组建军队` 按未锁定装备自动配发兵种，并按战士与民兵人数消耗行动力；可执行示例为 `组建军队 石刃 3 1`。`disbandarmy/解散军队` 归还装备并释放人口。`war targets/战争目标`、`power/战力` 展示占领目标与完整战力计算；占领后要用 `garrison/驻军` 维持据点。
...
存档位于 `saves/game`，含 `slot1.sav` 至 `slot6.sav` 及 `autosave.sav`。当前格式版本为 7；v5/v6 及更早版本会提示需要新开局，且不会修改原文件。
...
构建产物位于 `out/Formal-Debug` 或 `out/Formal-Release`。CTest 会执行 `tribe-formal-tests`；当前测试覆盖 v7 存档、旧版本拒绝、统一人口池、按人数派遣、负责人、装备品质、治疗休整、季度事件队列、资源贸易与玩家界面。

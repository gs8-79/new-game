...
> 正式课程作品《燧火纪：部落黎明》已经作为独立程序加入本仓库。Windows 双击 `开始正式版.cmd`，或查看 [正式版说明](formal/README.md)。原有三个 Demo 继续保留用于选题对照。
...
macOS 用户请阅读 [MACOS.md](MACOS.md)，并在终端中运行 `bash build-macos.sh Release` 与 `bash run-macos.sh Release`。

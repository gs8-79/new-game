...
namespace mud {
namespace {
class StarportPlaceholder final : public Scenario {
public:
    std::string id() const override { return "starport"; }
    std::string title() const override { return "星港危机：最后的维修员（待实现）"; }
...
std::unique_ptr<Scenario> makeStarportScenario() { return std::make_unique<StarportPlaceholder>(); }
} // namespace mud
